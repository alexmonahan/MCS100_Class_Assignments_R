...
# Rembering Calvin Johnson's career

# Earlier this year, Detroit Lions wide receiver Calvin Johnson announced his
# retirement, joining Barry Sanders as all-time Lions greats who hung up their
# spikes after their age-30 seasons, seemingly in the middle of their primes.
...
# vectors storing the year-by-year receiving yards and game totals for Johnson.

yards = c(756, 1331, 984, 1120, 1681, 1964, 1492, 1077, 1241)
games = c(15, 16, 14, 15, 16, 16, 14, 13, 16)
...
# example we can get Johnson's receiving yards per game in each season of his
...
# the years of Johnson's career by defining a vector from 2007 to 2015,
# counting by 1.
...
# of our year vector. Because Johnson was born in 1985, this gives us his age
# in each season of his career.

age = year - 1985
age

# Next let's plot Johnson's receiving yards per game by age:
...
# create a logical vector which flags seasons in which Johnson played 16 games.

games == 16

# Next, we can use this vector to return yards per game for seasons in which
# Johnson played at least 16 games.
...
# which Johnson played 16 games before 2015.
...
# As one last example, let's print the seasons in which Johnson averaged over
...
# length). For example, the command below returns Johnson's average receiving
# yards per year.

mean(yards)

# The sum() function adds up all of the elements in a vector. For example, the
# command below adds up all of Johnson's yards and divides by hist total number
...
# sort Johnson's season from most receiving yards per game to least.
...
data = read.csv('receiving2015.csv')
...
# Brent Celek is the leader, but this is dissatisfying because Celek only has
# 35 targets. Let's set the mininum threshold a little bit higher, at 80. The
...
# 80 targets, and we then extract the row corresponding to the league leader.

data[data$Tgt >= 80, ][which.max(data$YPT[data$Tgt >= 80]), ]
...
# Exercise 2:
# Who led all quarterbacks in receiving yards in 2015? Write your code below.

qb = data[data$Pos == 'QB', ]
head(qb[order(-qb$Yds), ])  # Marcus Mariota (41 yards)
...
aggregate(data$Yds, list(WR = data$Pos == 'WR', Team = data$Team), max)

# We observe that only Carolina, Cleveland, New England, San Diego, Tennessee
...
leaderYards = aggregate(data$Yds, list(Team = data$Team), max)
head(leaderYards[order(leaderYards$x), ])
data[data$Team == 'STL' & data$Yds == 681, ]    # Kenny Britt (STL), 681 yards
# Checkpoint 5: Did you complete Exercise 3?

# CHALLENGE 1
# Download receiving2014.csv from the class website and use it to plot 2015
# receiving yards per target against 2014 receiving yards per target for all
# players who have at least 30 targets in both seasons.
# Hint: ?merge
data14 = read.csv('receiving2014.csv')
merged = merge(data14, data, by = 'Receiver')
subset = merged$Tgt.x >= 30 & merged$Tgt.y >= 30
plot((merged$Yds.x/merged$Tgt.x)[subset], (merged$Yds.y/merged$Tgt.y)[subset])
# CHALLENGE 2
# Add up total receiving yards in 2015 for each unique first name in the
# dataset. For example, for "Michael" add up the receiving yards of Michael
# Crabtree, Michael Floyd, Michael Hoomanawanui, Michael Burton, Michael
# Campanero, and Michael Williams. Do not include Christine Michael or Mike
# Evans (or any other Mikes). Which first name accumulated the most receiving
# yards in 2015?
# Hint: ?strsplit
splitname = strsplit(as.character(data$Receiver), split = ' ')
